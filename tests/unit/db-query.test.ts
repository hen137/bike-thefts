...
type DBParam = Parameters<typeof createSchema>[0];
...
describe("queryHeatmap", () => {
  ...
  beforeEach(() => {
    ...
    createSchema(db as unknown as DBParam);
  });
  ...
  it("returns only records in date range", () => {
    insertRecords(db as unknown as DBParam, [
    ...
    const result = queryHeatmap(
      db as unknown as DBParam,
      "2020-01-01",
      "2021-12-31"
    );
  ...
  it("groups records at same coordinate into single HeatRow with correct count", () => {
    insertRecords(db as unknown as DBParam, [
  ...
  it("excludes records with null lat or lng", () => {
    insertRecords(db as unknown as DBParam, [
      makeMockRecord(1, { lat: 43.7, lng: -79.4, occ_date: "2021-06-01" })
    ]);
  ...
  it("returns empty array when no records match date range", () => {
    insertRecords(db as unknown as DBParam, [
  ...
  it("multiple distinct coordinates return separate HeatRows", () => {
    insertRecords(db as unknown as DBParam, [
